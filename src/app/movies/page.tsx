...
import { fetchMovieByFilter } from "../actions";
...
export default function MoviesPage() {
    ...
    const loadMovies = async (filters: SelectedFilters, reset = false) => {
        const loadingFunction = reset ? setIsLoading : setIsFetchingMore;
        loadingFunction(true);

        try {
            const newData = await fetchMovieByFilter({
                ...filters,
                page: reset ? 1 : page
            });
        ...
        <div className="max-w-7xl mx-auto px-4">
            <main >
                ...
                <section className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    ...
                    {movies.length === 0 && !isLoading && !isFetchingMore && (
                        <div className="col-span-full text-center py-10 text-zinc-400">
                            {selectedFilters.genre || selectedFilters.country || selectedFilters.year
                                ? "No movies found with these filters"
                                : "Apply filters to see movies"}
                        </div>
                    )}
                </section>

                {/* Trigger para o infinite scroll */}
